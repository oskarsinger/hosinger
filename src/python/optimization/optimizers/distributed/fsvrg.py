...
class RLFSVRGNode:
    ...
    def _compute_local_fsvrg_params(self):
        ...
        self.n_jks = self.get_coord_counts(self.rewards)
        self.phi_jks = self.n_jks / self.nk

    def get_update(self, global_w, global_grad, num_rounds):
        ...
        for i in xrange(num_rounds):
            self.nk += 1
            action_i = self.get_action(datum_i, w_i)

            self.actions.append(action_i)

            reward_i = self.server.get_reward(action_i)

            self.rewards.append(reward_i)

            datum_i = (action_i, reward_i)
