...
def test_two_fixed_rate_scalar(
    ...
    bs = cca_k + icl(cca_k)
    dl1 = FRL(dir_path, file1, seconds, reader1, online=True)
    dl2 = FRL(dir_path, file2, seconds, reader2, online=True)
    ds1 = M2M(dl1, bs)
    ds2 = M2M(dl2, bs)

    (Phi, unn_Phi, Psi, unn_Psi) = test_online_appgrad(
        ds1, ds2, cca_k)
...
def test_n_fixed_rate_scalar(
    ...
    (basis_pairs, Psi) = test_online_n_view_appgrad(
        dss, cca_k)
